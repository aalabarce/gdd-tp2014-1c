...
SELECT * 
FROM 
(SELECT (PUB_STOCK - (SELECT (isnull(sum(COM_CANTIDAD),0)) FROM STR_NOMBRE_GRUPO.COMPRA WHERE COM_PUB_ID = PUB_ID)) as PUB_STOCK, PUB_PRECIO, PUB_DESCRIPCION, PUB_USU_ID, PUB_ID, ROW_NUMBER() OVER (ORDER BY PUB_VIS_ID) as rowNum 
FROM STR_NOMBRE_GRUPO.PUBLICACION 
...
AND (PUB_ID IN (SELECT PUB_RUB_PUB_ID FROM STR_NOMBRE_GRUPO.PUBLICACION_RUBRO WHERE PUB_RUB_RUBRO_ID = @rubroId) OR @rubroId IS NULL) 
AND (PUB_STOCK - (SELECT (isnull(sum(COM_CANTIDAD),0)) FROM STR_NOMBRE_GRUPO.COMPRA WHERE COM_PUB_ID = PUB_ID)) > 0 ) sub 
WHERE rowNum > @contador AND rowNum <= @contador + 10
 
GO
	
	
CREATE PROCEDURE [STR_NOMBRE_GRUPO].[OfertasLIMIT] 
@contador int, @rubroId int = null, 
@descripcion varchar(4000) = null 
AS 
SELECT * 
FROM 
(SELECT PUB_STOCK, PUB_PRECIO, PUB_DESCRIPCION, PUB_USU_ID, PUB_ID, ROW_NUMBER() OVER (ORDER BY PUB_VIS_ID) as rowNum 
FROM STR_NOMBRE_GRUPO.PUBLICACION 
WHERE PUB_ESTADO_ID = 'A' AND PUB_STOCK > 0 
AND PUB_FECHA_FINALIZACION > GETDATE() 
AND PUB_TIPO_ID = 'S' 
AND PUB_ID NOT IN (SELECT COM_PUB_ID FROM STR_NOMBRE_GRUPO.COMPRA) 
AND PUB_DESCRIPCION LIKE '%' + @descripcion + '%' 
AND (PUB_ID IN (SELECT PUB_RUB_PUB_ID FROM STR_NOMBRE_GRUPO.PUBLICACION_RUBRO WHERE PUB_RUB_RUBRO_ID = @rubroId) OR @rubroId IS NULL)) sub 
WHERE rowNum > @contador AND rowNum <= @contador + 10 
