...
as
SELECT Usuario as Usuario, Año, Trimestre, Visibilidad, SUM(CANTIDAD)as Cantidad
FROM(
SELECT PUB_ID, Usuario, Año, Trimestre, Visibilidad, Stock - SUM(COM_CANTIDAD) as CANTIDAD
FROM
(SELECT PUB_ID, USU_USERNAME as Usuario,VIS_DESCRIPCION as Visibilidad, PUB_STOCK  As Stock, COM_CANTIDAD, YEAR(PUB_FECHA_INICIO) as Año,
...
JOIN STR_NOMBRE_GRUPO.USUARIO ON PUB_USU_ID=USU_ID
JOIN STR_NOMBRE_GRUPO.VISIBILIDAD ON VIS_ID=PUB_VIS_ID
WHERE COM_ID > 92750) tablita
GROUP BY PUB_ID, Usuario, Año, Trimestre, Visibilidad, Stock
HAVING Stock - SUM(COM_CANTIDAD) > 0) tab
GROUP BY Usuario, Año, Trimestre, Visibilidad
...
PUB_PRECIO, PUB_DESCRIPCION, PUB_USU_ID, PUB_ID, ROW_NUMBER() OVER (ORDER BY VIS_PRECIO DESC) as rowNum
FROM STR_NOMBRE_GRUPO.PUBLICACION, STR_NOMBRE_GRUPO.VISIBILIDAD      
WHERE PUB_TIPO_ID = 'C'
 AND (PUB_ESTADO_ID = 'A' OR PUB_ESTADO_ID = 'P')
 AND PUB_DESCRIPCION LIKE '%' + @descripcion + '%'
 AND (PUB_ID IN (SELECT PUB_RUB_PUB_ID 
                 FROM STR_NOMBRE_GRUPO.PUBLICACION_RUBRO 
				 WHERE PUB_RUB_RUBRO_ID = @rubroId) OR @rubroId IS NULL)
 AND (PUB_STOCK - (SELECT (isnull(sum(COM_CANTIDAD),0)) 
				   FROM STR_NOMBRE_GRUPO.COMPRA 
				   WHERE COM_PUB_ID = PUB_ID)) > 0
 AND VIS_ID = PUB_VIS_ID 
 AND PUB_USU_ID <> @usuarioId) sub
...
SELECT * 
FROM 
(SELECT PUB_STOCK, isnull((SELECT MAX(OFE_MONTO) FROM STR_NOMBRE_GRUPO.OFERTA WHERE OFE_PUB_ID = PUB_ID),PUB_PRECIO) PUB_PRECIO, PUB_DESCRIPCION, PUB_USU_ID, PUB_ID, PUB_VIS_ID,
...
WHERE PUB_STOCK > 0 
 ...
 AND PUB_TIPO_ID = 'S' 
 AND PUB_ID NOT IN (SELECT COM_PUB_ID FROM STR_NOMBRE_GRUPO.COMPRA) 
 AND PUB_DESCRIPCION LIKE '%%' 
 AND (PUB_ID IN (SELECT PUB_RUB_PUB_ID 
				 FROM STR_NOMBRE_GRUPO.PUBLICACION_RUBRO 
				 WHERE PUB_RUB_RUBRO_ID = NULL) OR NULL IS NULL) 
 AND VIS_ID = PUB_VIS_ID
 AND PUB_USU_ID <> @usuarioId ) sub  
WHERE rowNum > @contador 
 AND rowNum <= @contador + 10
